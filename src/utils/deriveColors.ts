...
export const deriveColors = (state: StateType) => {
  return state.scales.map((scale, scaleIndex) => {
    return scale.chromas.map((chroma, shadeIndex) => {
      const bgColorLCH = { l: luminances[shadeIndex], c: chroma, h: scale.hue }
      const color = lch(bgColorLCH)
      const bgColorHex = color._rgb._clipped ? undefined : color.hex()
